...
def my_softmax(values: np.ndarray, T=1.):
    ...
    """
    probas = np.exp((values - np.max(values)) / T) / np.sum(np.exp((values - np.max(values)) / T))
...
class EVSarsaAgent(QLearningAgent):
    ...
    def get_value(self, state):
        ...
        value = sum(prob * self.get_qvalue(state, action) for action, prob in zip(possible_actions, self.get_softmax_policy(state)))
