...
def predict_probs(states, model):
    ...
    with torch.no_grad():
        data = torch.FloatTensor(states)
        probs = torch.softmax(model(data), 1).numpy()
...
def get_loss(logits, actions, rewards, n_actions=n_actions, gamma=0.99, entropy_coef=1e-2):
    ...
    log_probs_for_actions = (log_probs * to_one_hot(actions, n_actions)).sum(dim=1) # [batch,]
    assert log_probs_for_actions is not None, "log_probs_for_actions is not defined"
    J_hat = (cumulative_returns * log_probs_for_actions).mean()  # a number
